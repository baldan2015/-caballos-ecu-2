<?php  session_start();
   ...
   if (file_exists("../entidad/Constantes.php")) {        include_once("../entidad/Constantes.php");    }
    ...
    if(isset($_POST["opc"])){
        ...
        echo  listarEjemplar($prefijo,$nombre,$fecNace,$fecFallece,$idPelaje,$LugarNace,$microchip,$adn);
            ...
            $tieneBasica=$_POST["tieneBasica"];
               //print_r( $propie);
                echo insertar($codigo,$prefijo,$nombre,$fecNace,$padre,$madre,$idPelaje,$lugarNace,$microchip,$adn,$descripcion,$usuario_crea,$genero,$fecCapado,$fecFallece,$motivoFallece,$idProvincia,
                     $origen,$resenias,$fecReg,$nroLibro,$nroFolio,$fecServ,$metodo,$reseniaBasica,$tieneBasica);
            ...
            $reseniaBasica=$_POST["reseniaBasica"];
            $tieneBasica=$_POST["tieneBasica"];
            echo editar($codigo,$prefijo,$nombre,$fecNace,$padre,$madre,$idPelaje,$lugarNace,$microchip,$adn,$descripcion,$usuario_modi, $genero,$fecCapado,$fecFallece,$motivoFallece,$idProvincia,$resenias,$fecReg,$nroLibro,$nroFolio,$fecServ,$metodo,$origen,$reseniaBasica,$tieneBasica);
    ...
    function insertar($codigo,$prefijo,$nombre,$fecNace,$padre,$madre,$idPelaje,$lugarNace,$microchip,$adn,$descripcion,$usuario_crea,$genero,$fecCapado,$fecFallece,$motivoFallece,$idProvincia,$origen,$resenias,$fecReg,$nroLibro,$nroFolio,$fecServ,$metodo,$reseniaBasica,$tieneBasica){
          $retorno=new Resultado();  
        ...
        //if(strlen($nombre)>0 && strlen($prefijo) >0 ){
                ...
                $response = $ins->insertar($codigo,$prefijo,$nombre,$fecNace,$padre,$madre,$idPelaje,$lugarNace,$microchip,$adn,$descripcion,$usuario_crea,$genero,$fecCapado,$propietarios,$criadores,$fecFallece,$motivoFallece,$idProvincia,$origen,$resenias,$fecReg,$nroLibro,$nroFolio,$fecServ,$metodo,$reseniaBasica);
   ...
   //}
                   ...
                   // print_r($result->idResenias);
                    ...
                    if($result->propietarios!=null){
                        foreach ($result->propietarios as $key => $propietario) {
                             $_SESSION['_datosProp'][]=array('codigo'=>$propietario->codigo,
                    ...
                    if($result->criadores!=null){
                         foreach ($result->criadores as $key => $criador) {
                             $_SESSION['_datosCri'][]=array('codigo'=>$criador->codigo,
                   ...
                   if($result->idResenias!=null){
                    ...
                    foreach ($res as $key => $resenas) {
                       // echo $key."gg".$resenas."<br>";
                         ...
                         // print_r($re);
                             $_SESSION['_datosRes'][]=array('id'=>$resenas,
    ...
    //Editar
    function editar($codigo,$prefijo,$nombre,$fecNace,$padre,$madre,$idPelaje,$lugarNace,$microchip,$adn,$descripcion,$usuario_modi,$genero,$fecCapado,$fecFallece,$motivoFallece,$idProvincia,$resenias,$fecReg,$nroLibro,$nroFolio,$fecServ,$metodo,$origen,$reseniaBasica,$tieneBasica){
         $retorno=new Resultado();
         if(validarSesion($retorno)->result==1){
        ...
        $response = $edi->editar($codigo,$prefijo,$nombre,$fecNace,$padre,$madre,$idPelaje,$lugarNace,$microchip,$adn,$descripcion,$usuario_modi,$genero,$fecCapado,$propietarios,$criadores,$propietariosDEL,$criadoresDEL,$fecFallece,$motivoFallece,$idProvincia,$resenias,$fecReg,$nroLibro,$nroFolio,$fecServ,$metodo,$origen,$reseniaBasica);
    ...
    function setResenas($resenas){
       ...
       foreach ($resenas as $key => $resenas) {
                             $_SESSION['_datosRes'][]=array('id'=>$resenas->id,
