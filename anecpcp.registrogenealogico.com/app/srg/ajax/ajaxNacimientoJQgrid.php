<?php
   ...
   // $prop=$_GET['cboProp']; //:$("#dllProp").val(),
    ...
    $count = $ejemplarServicio->numeroRegistroNacimiento($idEjemplar,$prefijo,$nombre,$prop,$vestado,$ente);//$row['count'];
    // echo $count." ..<br>";
    if( $count >0 )
    ...
    $i=0;
     ...
     $resultado=$ejemplarServicio->buscarSearchNacimiento($idEjemplar,$prefijo,$nombre,$prop,$vestado,$ente,$start,$limit,$sidx,$sord);
// echo "<pre>";     print_r($resultado);
     foreach ($resultado as $key => $fila) {
        $responce->rows[$i]['id']=$fila->codigo;
        $responce->rows[$i]['cell']=array($fila->codigo,
                                          $fila->codigoNacimiento,
                                          $fila->prefijo,
                                          $fila->nombre,
                                          $fila->fecNace,
                                          $fila->fecCrea,                                          
