<?php  session_start();
   ...
   if (file_exists("../entidad/Constantes.php")) {        include_once("../entidad/Constantes.php");    }
    ...
    if(isset($_POST["opc"])){
        ...
        echo  listarEjemplar($prefijo,$nombre,$fecNace,$fecFallece,$idPelaje,$LugarNace,$microchip,$adn);
            ...
            $codigoGenerado=$_POST["codigoGenerado"];
               ...
               //print_r( $propie);
                echo insertarNac($codigoIns,$prefijo,$nombre,$fecNace,$padre,$madre,$idPelaje,$lugarNace,$microchip,$adn,$descripcion,$genero,$usuModi,$fecCapado,$idMonta,$idNac,$idProvincia,$origen,$resenias,$fecModi,$nroLibro,$nroFolio,$fecServ,$metodo,$idProp,$idPoe,$idCriador,$arrayResenias,$codigoGenerado);
            ...
            echo editarNac($codigo,$prefijo,$nombre,$fecNace,$padre,$madre,$idPelaje,$lugarNace,$microchip,$adn,$descripcion,$genero,$usuModi,$fecCapado,$idMonta,$idProvincia,$origen,$resenias,$fecModi,$nroLibro,$nroFolio,$fecServ,$metodo,$idProp,$idPoe,$idCriador,$arrayResenias,$codigoGenerado, $idImgEliminacion, $idPdfEliminacion,$reseniaBasica);
            

            
        } elseif($_POST["opc"]=="del"){
            $codigo = $_POST["key"];
            echo eliminarNAC($codigo,$usuario_modi);
        } elseif($_POST["opc"]=="get"){
             $codigo = $_POST["codigo"];
            echo obtenerIDNAC($codigo);
           ...
           echo listaComboMtdoReprop($id,$descripcion);
            ...
            echo getEstadosLogNacimiento($id);
             ...
             $prop=$_POST["prop"];
            echo obteneDatosInscripcionPrint($id,$codigoNacimiento,$prop);
            ...
            $vProp=$_POST["vProp"];
            $vCria=$_POST["vCria"];
            echo actualizarEstadoSolNac($id,$estado,$comentario,$idProp,$vProp,$vCria);
        }else if($_POST["opc"]=="allNac"){
            echo cantidadAllNacimientos();   
        ...
        }else if($_POST["opc"]=="getImgNac"){
            $codigo=$_POST["codigo"];
            $esPdf=$_POST["esPdf"];
            echo listarImgNac($codigo,$esPdf);
        }else if($_POST["opc"]=="getDocNac"){
            $codigo=$_POST["codigo"];
            $esPdf=$_POST["esPdf"];
            echo listarDocNac($codigo,$esPdf);
        }
    ...
    function insertarNac($codigo,$prefijo,$nombre,$fecNace,$padre,$madre,$idPelaje,$lugarNace,$microchip,$adn,$descripcion,$usuario_crea,$genero,$fecCapado,$idMonta,$idProvincia,$origen,$resenias,$fecReg,$nroLibro,$nroFolio,$fecServ,$metodo,$idProp,$idPoe,$idCriador,$arrayResenias,$codigoGenerado){
          $retorno=new Resultado(); 
                ...
                $response = $ins->insertarNac($codigo,$prefijo,$nombre,$fecNace,$padre,$madre,$idPelaje,$lugarNace,$microchip,$adn,$descripcion,$usuario_crea,$genero,$fecCapado,$idMonta,$idProvincia,$origen,$resenias,$fecReg,$nroLibro,$nroFolio,$fecServ,$metodo,$idProp,$idPoe,$idCriador,$arrayResenias,$codigoGenerado);
   ...
   //}
    ...
    //Eliminar
    function eliminarNAC($codigo,$usuario_modi){
         $retorno=new Resultado();
       // echo $codigo;
         $objDel = new EjemplarLogica();
         //if(validarSesion($retorno)->result==1){
        
        $response = $objDel->eliminarNAC($codigo,$usuario_modi);
   ...
   // }
        ...
        $retorno=new Resultado();
             ...
             if(validarSesion($retorno)->result==1){    
                $obj = new EjemplarLogica();   
                   $result=$obj->obtenerIDNAC($codigo);
    ...
    //Editar
    function editarNac($codigo,$prefijo,$nombre,$fecNace,$padre,$madre,$idPelaje,$lugarNace,$microchip,$adn,$descripcion,$genero,$usuModi,$fecCapado,$idMonta,$idProvincia,$origen,$resenias,$fecModi,$nroLibro,$nroFolio,$fecServ,$metodo,$idProp,$idPoe,$idCriador,$arrayResenias,$codigoGenerado, $idImgEliminacion, $idPdfEliminacion,$reseniaBasica){
         $retorno=new Resultado(); 
              $retorno->validateTOKEN="1";
              if(validarSesion($retorno)->result==1){      
                $ins = new EjemplarLogica();
                ...
                $resenias=$codResenas;
                    ...
                    if ($response->result == 1){
                        ...
                        if (is_array($ides)) {
                            ...
                            $delete = $imgDelete->eliminarNacTMP($ides);
                        }
                        ...
                        if (is_array($idesPdf)) {
                           foreach ($idesPdf as $key => $id) {
                            ...
                            $delete = $imgDelete->eliminarNacTMP($idesPdf);
                        }
...
function setResenas($resenas)
{
    ...
    function obteneDatosNacimientoPrint($id,$codigoInscripcion,$prop){
              $retorno=new Resultado();
              $obj=new  EjemplarLogica();
              $retorno->validateTOKEN="1";
              //$retorno->data=$obj->obteneDatosNacimientoPrint($id,$codigoNacimiento,$prop);
              $result=$obj->obteneDatosNacimientoPrint($id,$codigoInscripcion,$prop);
         ...
         function actualizarEstadoSolNac($id,$estado,$comentario,$idProp,$vProp,$vCria){
            $retorno=new Resultado(); 
              $retorno->validateTOKEN="1";
              if(validarSesion($retorno)->result==1){      
                $ins = new EjemplarLogica();

                    $response = $ins->actualizarEstadoSolNac($id,$estado,$comentario,$idProp,$vProp,$vCria);
                   // echo strlen($response->result);
                    if($response->result == 1){
                        ...
                        $retorno->result=999;
                        $retorno->message = Constantes::k_MENSAJE_VALIDADOR_NAC;
                    }else if($response->result==998){
                        $retorno->result=0;
                        $retorno->message=Constantes::K_MENSAJE_NOMBRE_SUPER_CAMP;                    
                    }
       ...
       function cantidadAllNacimientos(){
         ...
         if(validarSesion($retorno)->result==1){    
            ...
            $response=$list->cantidadAllNacimientos();
            //print_r($response);
        ...
        function listarImgNac($codigo,$esPdf){
            ...
            if(validarSesion($retorno)->result==1){    
                $result=$obj->buscarSearchNacTMP($codigo,$esPdf,$codigoGenerado);
                if(is_array($result)){
                    ...
                    foreach($result as $key => $value){
                       ...
                       $html.="<a href='".K_PATHWEB_NAC_IMG.$value->ruta."' target='_blank'><img src='".K_PATHWEB_NAC_IMG.$thumb_prefix.$value->ruta."' alt='Thumbnail' style=' cursor:pointer;' /></a>";
        ...
        function listarDocNac($codigo,$esPdf){
            ...
            $thumb_prefix           = "thumb_"; //Normal thumb Prefix
            if(validarSesion($retorno)->result==1){    
                $result=$obj->buscarSearchNacTMP($codigo,$esPdf,'');
                if(is_array($result)){
                        $html.="<div class='col-md-1' style='margin-top: 7px;'>
                                <label style='color: #b7adad;'>Documentos:</label></div>";
                    foreach($result as $key => $value){
                       $html.="<div class='col-md-1' style='margin-top: 7px;margin-left:10px;'>";
                       $html.="<a href='".K_PATHWEB_NAC_PDF.$value->ruta."' target='_blank'><img  src='images/icono/pdf.png' alt='Thumbnail' style=' cursor:pointer; width:30px;' title='".$value->idTipoDocumento."'/></a>"; 
