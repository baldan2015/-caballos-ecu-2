...
class PDFTable extends tFPDF
{
	...
	{
		$this->_makePageSize();
		$this->Image("../../images/anecpcp.jpeg", NULL, NULL, 93, 16, "JPEG");

		$this->SetFont('Helvetica', 'B', 10);
		//$this->SetXY(135, 10);
		$this->Cell(314, -20,utf8_decode($this->headerTable), 0, 0, 'C');
		
		$this->Rect(10, 22, 190, 0, 'D');
	...
	function Footer()
	{
		...
		$this->SetFont('Helvetica', 'B', 10);
		$titulo = utf8_decode("SISTEMA DE REGISTRO GENEALÓGICO - ANECPCP");
		
		$this->SetY(-15);
		$this->Rect(10, 282, 190, 0, 'D');
		$this->Cell(90, 10, $titulo, 0, 0, 'R');
	...
	private function _tableParser(&$html)
	{
		...
		foreach ($t->name as $i => $element) {
			if (
				$fontopen && $t->type[$i] == NODE_TYPE_ENDELEMENT
				&& (in_array($element, array('table', 'tr', 'td', 'font')))
			)
				$fontopen = false;
			if (
				$tdopen && $t->type[$i] == NODE_TYPE_ENDELEMENT
	...
	private function _tableWrite(&$table)
	{
		...
		if (
			isset($table['nobreak']) && $table['nobreak']
