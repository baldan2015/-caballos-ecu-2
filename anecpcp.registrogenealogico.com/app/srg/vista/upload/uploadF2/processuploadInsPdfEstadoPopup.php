...
$idTipoDocumento=$_POST['idTipoDocE'];
...
$destination_folder		= K_PATH_INS_PDF; //upload directory ends with / (slash)
...
if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
	...
	if(!isset($_FILES['pdf_fileE']) || !is_uploaded_file($_FILES['pdf_fileE']['tmp_name'])){
			die('Image file is Missing!'); // output error when above checks fail.
	}
 
	//uploaded file info we need to proceed
	$image_name = $_FILES['pdf_fileE']['name']; //file name
	$image_size = $_FILES['pdf_fileE']['size']; //file size
	$image_temp = $_FILES['pdf_fileE']['tmp_name']; //file temp
	$image_type = $_FILES['pdf_fileE']['type']; //file temp
	...
	if($image_res){
		...
		if(move_uploaded_file($image_temp, $image_save_folder))
		{
			...
			$ins = new ImagenLogica();
                ...
                $response = $ins->insertarInsTMP($idHorse,$new_file_name,$esPrincipal=0,$activo=1,1,$idTipoDocumento,$codigoGenerado,$flag);
			...
			echo '<div align="left">';
			echo '<input type="hidden" id="txtFlatInsert" value="1">';
			//echo '<br />';
			//require("../../../ajax/ajaxImagenIns.php");
			//echo listarPdfEjemplarIns($idHorse);
			echo '</div>';
