...
if (!validarSesion2()) {
    ...
    $ejemplar = $print->obteneDatosNacimientoPrint($_GET['codigo'], $_GET['codigoNacimiento'], $_GET['prop'],0);
    $vpadre;
    $vmadre;
    $vnombreAbueloPadre = '';
    $vnombreAbuelaPadre = '';
    ...
    foreach ($ejemplar as $key => $value) {
        ...
        if($resenasGeneral==""){
          $reseniaBasica = $print->obtenerIDNAC($_GET['codigo']);
    ...
    if (is_array($imagenes)) {
        foreach ($imagenes as $key => $fila) {
            if(file_exists(ConstantesPathWeb::K_PATH_NAC_IMG . $fila->ruta)){
              if(substr($fila->ruta,-3) == "png"){
                $imagenesCaballo .= '<table align="center" valign="middle"><tr><td><img src="' . ConstantesPathWeb::K_PATH_NAC_IMG . $fila->ruta . '" height=450 width=500></td></tr></table>' . $espacio;
              }else{
                $imagenesCaballo .= '<table align="center" valign="middle"><tr><td><img src="' . ConstantesPathWeb::K_PATH_NAC_IMG . $fila->ruta . '" height=117 width=147></td></tr></table>' . $espacio;
              }
