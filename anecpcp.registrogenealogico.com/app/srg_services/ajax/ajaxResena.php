...
// echo getcwd()." - ";
    ...
    */
        ...
        if($_POST["opc"]=="lst"){
         ...
         echo insertar($descripcion,$usuario_crea);
        }else if($_POST["opc"]=="upd"){
            $id          = $_POST["id"];
            $descripcion =addslashes($_POST["descripcion"]);            
            echo editar($id,$descripcion,$usuario_modi);
           ...
           $codigo="";//$_POST["codigo"];
           $descripcion="";//$_POST["descripcion"];
           echo listarComboResenias($codigo,$descripcion);
        }
    }
        if(isset($_GET["opc"])){
            if($_GET["opc"]=="jqgrid"){
    ...
    function insertar($descripcion,$usuario_crea){
        $retorno=new Resultado();
        if(validarSesion($retorno)->result==1){ 
        $ins = new ResenaLogica();
        if(strlen($descripcion)>0){
                $response = $ins->insertar($descripcion,$usuario_crea);
                //print_r($response);
   ...
   /* function generarCodigo($codigo_empresa){
    ...
    //Editar
    function editar($id,$descripcion,$usuario_modi){
         $retorno=new Resultado();
         if(validarSesion($retorno)->result==1){ 
        $edi = new ResenaLogica();
        $response = $edi->editar($id,$descripcion,$usuario_modi);
        if ($response->result == 1){
            $retorno->result=$response->result;
    ...
    $i=0;
     ...
     $resultado=$entidadServicio->buscarSearch($nomResena,$start,$limit,$sidx,$sord);
    // print_r ($resultado);
  ...
  function listarComboResenias($codigo,$descripcion){

        $retorno=new Resultado();
        $obj=new ResenaLogica();
        $result = $obj->listarCombo($codigo,$descripcion);
