    ...
    if($_SERVER["REQUEST_METHOD"]=='POST'){
                   ...
                   if(is_object($response)){
                    ...
                    echo json_encode($retorno);
                     ...
                     if($retorno->result==1){
                        ...
                        $mes=$_POST["mes"];
                        if($retorno->result==1){
                            echo listarServicioY($idPoe, $idProp,$metodoReprod,$idReceptora,$madre,$padre,$anio,$mes);
                        }else{
                            header("HTTP/1.1 200 OK");
                            echo json_encode($retorno);
                        }
      ...
      //unset($_SESSION['_datosRes']);
       ...
       foreach ($resenas as $key => $resenas) {
        ...
        function listarServicioY($idPoe, $idProp,$metodoReprod,$idReceptora,$madre,$padre,$anio,$mes){
                     $retorno=new Resultado();
                     $servicio = new EjemplarLogica();
                     $retorno=new Resultado(); 
                     $retorno->validateTOKEN="1";
                      $retorno->message=Constantes::K_MENSAJE_SELECT_NOOK;
                     if(validarSesion($retorno)->result==1){    
                        $retorno->data=$servicio->listarServicioY($idPoe, $idProp,$metodoReprod,$idReceptora,$madre,$padre,$anio,$mes);
                        $retorno->cantidad=sizeof($retorno->data);
                        $retorno->message=Constantes::K_MENSAJE_SELECT_OK;
                      }
