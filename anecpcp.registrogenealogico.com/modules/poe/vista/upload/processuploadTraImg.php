...
$idHorse=$_POST['idHorseT'];
$idTipoDocumento=$_POST['idTipoDocT'];
...
$destination_folder		= K_PATH_TRANS_IMG; //upload directory ends with / (slash)
...
if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
	...
	$image_type = $_FILES['image_file']['type']; //file temp

	if($image_type=='application/pdf')$isPDF=true;
		...
		if(!$isPDF){
					if(normal_resize_image($image_res, $image_save_folder, $image_type, $max_image_size, $image_width, $image_height, $jpeg_quality))
					{
						...
						/*insert tabla galeria*/
			                ...
			                $response = $ins->insertarTraTMP($idHorse,$new_file_name,$esPrincipal=0,$activo=1,0,$idTipoDocumento);
						...
						echo '<div align="left">';
						echo '<input type="hidden" id="txtFlatInsert" value="'.$response->result.'">';
						echo '</div>';
				...
				if(move_uploaded_file($image_temp, $image_save_folder))
				{
					...
					$ins = new ImagenLogica();
		                ...
		                $response = $ins->insertarTraTMP($idHorse,$new_file_name,$esPrincipal=0,$activo=1,1,$idTipoDocumento);
		                if ($response->result == 1){
		                   echo Constantes::K_MENSAJE_INSERT_OK;
		                }else if($response->result == 999){
		                   echo Constantes::K_MENSAJE_VALIDACION_INS;
		    			...
		    			echo "¡fichero no es válido para subir al servidor!\n";
				}
		}
