<?php  session_start();
    if (file_exists("../logica/ImagenLogica.php")) {  include_once ("../logica/ImagenLogica.php");}
...
if(!isset($_POST["opc"]))   {
   ...
   // echo listarEjemplar('','','','','','','','');
     ...
     */
        ...
        if($_POST["opc"]=="lst"){
            ...
            echo  listarPdfEjemplarNac($id,$codigoGenerado,$edit);
             ...
             $codigoGenerado = $_POST["codigoGenerado"];
             $edit=$_POST["edit"];
            echo  listarImagenEjemplarIns($id,$codigoGenerado,$edit);
        ...
        }else if($_POST["opc"]=="dltDocNAC"){
            $codigo =   $_SESSION["xid"];
            $esPDF = $_POST["esPDF"];
            echo eliminarDocumentosNAC($codigo,$esPDF);
        }else if($_POST["opc"]=="dltDocINS"){
            $codigo =   $_SESSION["xid"];
            $esPDF = $_POST["esPDF"];
            echo eliminarDocumentosINS($codigo,$esPDF);
        }else if($_POST["opc"]=="lstDocMonta"){
            $idMonta =   $_POST["idMonta"];
            echo listarDocumentosMonta($idMonta);
        }
    ...
    function eliminarDocumentosNAC($codigo,$esPDF){
        $retorno=new Resultado();
        $objImagen= new ImagenLogica();
        $result = $objImagen->eliminarDocumentosNAC($codigo,$esPDF);
...
//Editar
    function editar($codigo,$main,$idHorse){
        ...
        if ($response->result == 1){
            ...
            $retorno->result=999;
            $retorno->message = "No se puede agregar o editar una imagen ya que la inscripción del ejemplar se encuentra aprobada";
...
*/
 ...
 function listarImagenEjemplarNac($id,$codigoGenerado){
...
//        $html.= "<th  >Es principal</th>";
        ...
        foreach ($datos as $key => $fila) {
            $botonHtml="<span class='btnDel btn btn-sm btn-default glyphicon glyphicon-trash' style=' cursor:pointer;'
             onclick=eliminarImg(".$fila->id.",'".$fila->idNacimiento."','".$codigoGenerado."'); id='btnDelE' ></span>";
           /* $isMain=$fila->esPrincipal==1?"SI":"NO";
            $botonHtmlEsprincipal="<label class='btnUpd' onclick=updPrincipal(".$fila->id.",".$fila->esPrincipal.",'".$fila->idCaballo."'); style=' cursor:pointer;' >".$isMain."</label>";
           */
...
*/
  ...
  function listarPdfEjemplarNac($id,$codigoGenerado){
        $thumb_prefix           = "thumb_"; //Normal thumb Prefix
        ...
        $html.= "<th id='pdfAd' style='width:30%;'>PDF Adjuntado</th>";
//        $html.= "<th  >Es principal</th>";
        $html.= "<th id='acc' style='height:20px; '>...</th>";
        $html.= "</tr>";
        $html.= "</thead>";
        $html.= "</table>";
        ...
        foreach ($datos as $key => $fila) {
            $botonHtml="<span class='btnDel btn btn-sm btn-default glyphicon glyphicon-trash' style=' cursor:pointer;'
             onclick=eliminarPdf(".$fila->id.",'".$fila->idNacimiento."','".$codigoGenerado."'); ></span>";
           /* $isMain=$fila->esPrincipal==1?"SI":"NO";
            $botonHtmlEsprincipal="<label class='btnUpd' onclick=updPrincipal(".$fila->id.",".$fila->esPrincipal.",'".$fila->idCaballo."'); style=' cursor:pointer;' >".$isMain."</label>";
           */
...
*/
                ...
                $html.= "<td >
                 <img class='imgpdf'  src='../../../images/icono/pdf.png' alt='Thumbnail' style=' cursor:pointer; width:30px;'  onclick=mostrarImgGrande('".K_PATHWEB_NAC_PDF.$fila->ruta."'); /> 
                </td>";
              /*  $html.= "<td  >
...
function listarImagenEjemplarIns($id,$codigoGenerado,$edit){
        ...
        $html.= "<thead>";
        $html.= "<tr>";
        //$html.= "<th style='width:20%;'>Id Imagen</th>";
        //$html.= "<th style='width:20%;'>Id Solicitud</th>";
        //$html.= "<th style='width:20%;'></th>";
        $html.= "<th style='text-align:center;width:30%;'>Fecha</th>";
        $html.= "<th style='text-align:center;width:50%;'>Imagen</th>";
       /* $html.= "<th>CodigoGenerado</th>";*/
        $html.= "<th style='height:20px; '>...</th>";
        $html.= "</tr>";
        $html.= "</thead>";
        $html.= "</table>";
...
*/
  ...
  function listarPdfEjemplarIns($id,$codigoGenerado,$edit){
        ...
        $html.= "<thead>";
        $html.= "<tr>";
        //$html.= "<th style='width:20%;'>Id Documento</th>";
        $html.= "<th style='text-align:center;width:40%;'>Tipo Documento</th>";
        //$html.= "<th style='text-align:center;'>Id Solicitud</th>";
        $html.= "<th style='text-align:center;width:20%;'>Fecha</th>";
        $html.= "<th id='acc' style='width:30%;'>PDF Adjuntado</th>";
//        $html.= "<th  >Es principal</th>";
        $html.= "<th style='height:20px; '>...</th>";
        $html.= "</tr>";
        $html.= "</thead>";
...
*/  
                ...
                <input type='hidden' class='cssItem' value='".$fila->fecha."'>
                 ...
                 $html.= "<td  >
                 <img class='imgpdf'   src='../../../images/icono/pdf.png' alt='Thumbnail' style=' cursor:pointer; width:30px;'  onclick=mostrarImgGrande2('".K_PATHWEB_INS_PDF.$fila->ruta."'); /> 
                </td>";
              /* $html.= "<td  >
                 <img  src='../../../images/icono/pdf.png' alt='Thumbnail' style=' cursor:pointer; width:30px;'  onclick=mostrarImgGrande2('".K_PATHWEB.$fila->ruta."'); /> 
                </td>";*/
     /*
                $html.= "<td align='center'>
                <label  id='txtActivo_".$fila->id."' >".$botonHtmlEsprincipal."</label>
...
function listarDocumentosMonta($idMonta)
{
    $objDocumento = new ImagenLogica();

    $datos = $objDocumento->listarDocumentosMonta($idMonta,'');
    $collection = [];
    $i=0;
    if(is_array($datos)){
        foreach ($datos as $key => $fila) {
