var K_PATH_ROOT="../";
$(function(){
 $("#btnPrint").on("click",function(){ 	viewForm(3,$("#hidIdProp").val(),$("#hidIdPoe").val());}).button({icons: { primary: "ui-icon-print" }});
...
function agregarItems(){
			...
			var filaHtml="	<tr>"+
							...
							"<td><input type='text' class='cssFecha2  cssItem'  style='width:80px;' name='txtFechAborto_"+fila+"' id='txtFechAborto_"+fila+"'/></td>"+
...
$('.cssFecha2').each(function(i, obj) {
	...
	$( obj).datepicker({ dateFormat: 'dd/mm/yy',changeMonth: true, changeYear: true ,
        ...
        setTimeout(function () {
	        inst.dpDiv.css({ top: rect.top + 20, left: rect.left + -90 });
        }, 0);
    }
...
text: false
					...
					if(evaluarModoVigenciaPOE_Grilla(obj,$("#hidVigencia").val(),$("#hidAnioPer").val())){
     						...
     						$(obj).on("click",function(){
     							...
     							$("#divResultBG").html("");
								$("#hidTipoParents").val("M6");	
     							if(ctrl.indexOf("Potro")!=-1){
     								$("#hidTipoParents").val("PO");	
     							}
     							_buscar();
							...
							$('.cssItem ', this).each(function() {
									...
									inputNameB =  $(this).attr("name");
							  		values =  $(this).val(); 
										...
										if(inputName.indexOf('txtFechAborto_')!=-1) servicio.fecAborto=values;
							});
