<?php session_start();
	...
	function listar($link,$idpoe,$idProp){
		...
		$html.= "<thead >";
		$html.= "<tr>";
		$html.= "<th style='width:20%;'>Yeguas de mi propiedad o yeguas de Terceros </th>";
		$html.= "<th style='width:30px;'>...</th>";
		$html.= "<th style='height:35px;width:20%;'>Potro reproductor</th>";		
		$html.= "<th style='width:30px;'>...</th>";
		$html.= "<th>Metodo Reproductivo</th>";
		$html.= "<th style='width:120px;'>Fecha Ultimo Servicio</th>";
		$html.= "<th style='width:100px;'><span title='sólo ingresar la fecha si el ejemplar abortó.'>Fecha de Aborto</span></th>";
		$html.= "<th style='width:30px;'>...</th>";
		$html.= "</tr>";
		$html.= "</thead>";
		$html.= "<tbody  >";
		
		$sql = " SELECT m.id,DATE_FORMAT(m.fecMonta,'%d/%m/%Y') as fecha,m.metodo,codYegua,codPotro,idPoe,
				...
				ifnull(pad.nombre,(select p.nombre COLLATE utf8_unicode_ci from poe_propiedad p where p.codEjemplar=m.codPotro and  idPoe=".$idpoe." and idUser='".$idProp."')) potro , isTE,DATE_FORMAT(m.fecParir,'%d/%m/%Y') as fecAborto
				from poe_servicioy m 
		...
		while($res = mysqli_fetch_array($result)){
				$botonHtml="<label class='btnDel btn  btn-sm btn-default glyphicon glyphicon-trash' data-key=".$res['id']."></label> ";
				if($finalizo==true){
					$botonHtml="";
				}
			...
			$html.= "<td align='center'><label  class='btnSearch btn  btn-sm btn-default glyphicon glyphicon-search' data-ctrlset='hidYegua_".$fila."' data-ctrlsetdisplay='txtYegua_".$fila."' > </label></td>";
			...
			$html.= "<td align='center'><label  class='btnSearch btn  btn-sm btn-default glyphicon glyphicon-search' data-ctrlset='hidPotro_".$fila."' data-ctrlsetdisplay='txtPotro_".$fila."' > </label></td>";
			$html.= "<td align='left'>
						   				
		 					<input type='radio' class='cssItem' name='metodo_".$fila."' id='rdbtnMN_".$fila."' value='MN' $checkedMN /> 
		 					<label for='rdbtnMN_".$fila."'  title='Monta Natural' class='thItem'>Monta Natural</label> 
		 					<input type='radio' class='cssItem' name='metodo_".$fila."' id='rdbtnSF_".$fila."' value='SF' $checkedSF /> 
		 					<label for='rdbtnSF_".$fila."' title='Semen Fresco'  class='thItem'>Semen Fresco</label>
		 					 <input type='radio' class='cssItem' name='metodo_".$fila."' id='rdbtnSR_".$fila."' value='SR' $checkedSR />
		 					 <label for='rdbtnSR_".$fila."' title='Semen Refrigerado' class='thItem'>Semen Refrig.</label> 
		 					 <input type='radio' class='cssItem' name='metodo_".$fila."' id='rdbtnSC_".$fila."' value='SC' $checkedSC /> 
		 					 <label for='rdbtnSC_".$fila."' title='Semen Congelado'  class='thItem'>Semen Cong.</label>
		 					 <input type='checkbox' class='cssItem'  name='CheckMet_".$fila."' id='rdbtnTE_".$fila."' value='TE' ".$checkedTE." /> 
		 					 <label for='rdbtnTE_".$fila."' title='Transferencias de Embriones'  class='thItem'>Transf. de Embriones</label> 
		 	 			 
					</td>";
			$html.= "<td>
			<input type='text'  class='cssFecha cssItem' style='width:80px;' name='txtFech_".$fila."' id='txtFech_".$fila."' value='".$res['fecha']."'/>
			</td>";
			$fecAborto=$res['fecAborto'];
			if($fecAborto=="00/00/0000"){
					$fecAborto="";
			}
			$html.= "<td>
			<input type='text'  class='cssFecha2 cssItem' style='width:80px;' name='txtFechAborto_".$fila."' id='txtFechAborto_".$fila."' value='".$fecAborto."'/>
			</td>";
		...
		$html.= "</tbody>";
		$html.= "<tfoot><tr><td colspan=8 style='height:30px; '><b>Total registros: ".($fila-1)."<b></td><tr></tfoot>";
	...
	function insertar($idPoe,$idProp, $servicios,$link){
		...
		if($resultDel){
			foreach ($servicios as $key => $value) {
					...
					$mysqldateAborto=null;
					if(strlen($value->fecAborto)==10){
						$dateAborto = explode('/', $value->fecAborto);
						$timeAborto = mktime(0,0,0,$dateAborto[1],$dateAborto[0],$dateAborto[2]);
						$mysqldateAborto = date( 'Y-m-d H:i:s', $timeAborto );
					}

			        $sql= "INSERT INTO  poe_servicioy  (fecServ,metodo,codYegua,codPotro,idPoe,idUser,fecCrea,isTE,fecAborto)VALUES ('".$mysqldate."', '".$value->metodo."', '".$value->codYegua."', '".$value->codPotro."', '".$idPoe."','".$idProp."',now(),".$isTE.",'".$mysqldateAborto."');";
		 		    //echo $sql."<br>";
		 		    $result = mysqli_query($link,$sql);
				    if($result){
						$contador++;
	...
	function eliminarServicio($idPoe,$idProp,$link){
 			...
 			$sql="DELETE FROM  poe_servicioy WHERE idPoe='".$idPoe."' AND idUser='".$idProp."'";
 		    $result = mysqli_query($link,$sql);
	...
	function eliminar($id,$link){
 			...
 			$sql="DELETE FROM  poe_servicioy WHERE id=".$id."";
 		    $result = mysqli_query($link,$sql);
	...
	function listarHtmlAdmin($link,$idpoe,$idProp){
		...
		$html .= "FORMULARIO 3 - ".$codPropietario;
		$html .= "</td>";
		$html .= "</tr>";
		$html .= "<tr>";
		...
		$html .= "<center>RELACION DE YEGUAS DE MI PROPIEDAD Y VIENTRES CEDIDOS A MI FAVOR SERVIDAS DURANTE EL A&Ntilde;O $periodo</center>";
		$html .= "</td>";
		$html .= "</tr>";
		$html .= "<table>";
		...
		$html.= "<th colspan=3 style='width:30%;'>Yeguas de mi propiedad y/o cedidas a mi uso</th>";
 
		$html.= "<th   colspan=3 style='width:30%;'>Potro Reproductor</th>";		
		$html.= "<th colspan=5>Metodo Reproductivo</th>";
		$html.= "<th style='width:100px;'>Fecha del Servicio</th>";
		$html.= "<th style='width:100px;'>Fecha Aborto</th>";
		$html.= "</tr>";
		...
		$sql = " SELECT m.id,DATE_FORMAT(m.fecServ,'%d/%m/%Y') as fecha,m.metodo,codYegua,codPotro,idPoe,
				...
				isTE,DATE_FORMAT(m.fecAborto,'%d/%m/%Y') as fecAborto
				from poe_servicioy m 
				left join datos220206 pad on( pad.codigo = m.codPotro) 
				left join datos220206 mad on(mad.codigo =m.codYegua)where  idPoe=".$idpoe." and idUser='".$idProp."' order by 1
				";
