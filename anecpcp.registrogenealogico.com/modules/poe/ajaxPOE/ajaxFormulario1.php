...
//	require("../Funciones/queries.php");
	...
	if($finalizo!=true){
		...
		$htmlth1.= "<th style='height:35px;width:20%;'>N° del Registro</th>";
		$htmlth1.= "<th style='height:35px;width:35%; text-align:center;'>acción</th>";
		$htmlth1.= "</tr>";
		$htmlth1.= "</thead>";
		$html.= "<tbody  >";
		...
		$sql = "SELECT DISTINCT id,codigo,prefijo,nombre,esNuevo,esBD 
  				FROM ( 
					...
					UNION ALL
				      	...
				      	INNER JOIN datos2202062 a ON a.codigo =p.codEjemplar
					...
					";	
			//	 	echo   " tiene poe anterior"; 
		}else{
			//echo "<br>no tiene poe<br>";
			$sql = " SELECT DISTINCT id,codigo,prefijo,nombre,esNuevo,esBD 
  				FROM ( 
  						SELECT 0 as id, a.codigo  AS codigo, 
  						a.prefij  AS prefijo, 
  						a.nombre  as nombre, 
  						'' esNuevo,false esBD 
  					 FROM datos2202062 a
		...
		while($res = mysqli_fetch_array($result)){
			...
			$html.= "<td align='center'>";
			$html.= "<button class='btnPorpiedad btn btn-sm btn-default' data-codigo=".$res['codigo']." data-nombre='".$ejemplar."' data-prefijo=".$res['prefijo']." data-fin=".$finalizo." >Mi prop.</button>";

			$html.= "&nbsp;&nbsp;<button class='btnFallecio btn  btn-sm btn-default' data-codigo=".$res['codigo']." data-nombre='".$ejemplar."' data-prefijo=".$res['prefijo']." data-fin=".$finalizo." >Falleci&oacute;</button>";

		/*	$html.= "&nbsp;&nbsp;<button class='btnTransferido btn btn-sm btn-default' data-codigo=".$res['codigo']." data-nombre='".$ejemplar."' data-prefijo=".$res['prefijo']." data-fin=".$finalizo." >Transf.</button>";*/

			$html.= "&nbsp;&nbsp;<button class='btnCastrado btn btn-sm btn-default' data-codigo=".$res['codigo']." data-nombre='".$ejemplar."' data-prefijo=".$res['prefijo']." data-fin=".$finalizo." >Castrado</button>";
	...
	//	$htmlTotal= "<tfoot><tr><td colspan=4 style='height:30px; '><b>Total ejemplares: ".($fila-1)."<b></td><tr></tfoot>";
		...
		$retorno->result=1;
 		$retorno->html=$htmlTabla;
    	$retorno->numRow=$fila>0?($fila-1):0;
    	$retorno->message='OK';
	...
	function listarHtml($link,$idPoe,$idUser,$idProp){
		...
		$sql1="Select propie,cod_criador,criador FROM datos2202062 where cod_propie='".$idProp."'";
		//echo $sql1;
		$result = mysqli_query($link,$sql1) or die("error listarHtml ".mysqli_error($link));
		while($res = mysqli_fetch_array($result)){
		...
		*/
		...
		$html.= "<center><div  id='lnkPrint'>
			<a href=#   >Imprimir</a></div></center>";
...
function listarPropiedadAgregado($link,$idPoe,$idUser,$idProp){
	...
	$finalizo=EnvioForm($link,$idPoe,$idUser);
		...
		$html.= "<thead >";
		$html.= "<tr>";
		$html.= "<th style='height:35px;width:10%;'>Prefijo</th>";
		$html.= "<th style='height:35px;width:35%;'>Nombre del Ejemplar</th>";
		$html.= "<th style='height:35px;width:25%;'>N° del Registro</th>";
		$html.= "<th style='height:35px;width:30%;text-align:center'>acción</th>";
		$html.= "</tr>";
		$html.= "</thead>";
		$html.= "<tbody  >";
		...
		while($res = mysqli_fetch_array($result)){
				...
				$html.= "<td align='center'>";
				// if($finalizo!=true){
				 	$html.= "<button class='btnQuitar btn btn-sm btn-default ' data-id=".$res['id']." data-fin=".$finalizo.">Quitar propiedad</button>";
				 //}
				$html.= "</td>";
				$html.= "</tr>";
				$fila++;
