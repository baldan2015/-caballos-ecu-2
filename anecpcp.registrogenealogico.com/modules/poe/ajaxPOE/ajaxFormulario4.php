<?php session_start();
	...
	function listar($link,$idpoe,$idProp){
		...
		$html.= "<thead >";
		$html.= "<tr>";
		$html.= "<th style='height:35px;width:20%;'>Potros de mi propiedad o potros de terceros </th>";
		$html.= "<th style='width:30px;'>...</th>";
		$html.= "<th style='width:20%;'>Yeguas de terceros</th>";
		$html.= "<th style='width:30px;'>...</th>";
		$html.= "<th>Metodo Reproductivo</th>";
		$html.= "<th style='width:150px;'>Fecha Ultimo Servicio</th>";
		$html.= "<th style='width:30px;'>...</th>";
		$html.= "</tr>";
		$html.= "</thead>";
		$html.= "<tbody  >";
		...
		$sql = " SELECT m.id,DATE_FORMAT(m.fecServ,'%d/%m/%Y') as fecha,m.metodo,codYegua,codPotro,idPoe,
				...
				left join datos2202062 pad on( pad.codigo  = m.codPotro) 
				left join datos2202062 mad on(mad.codigo  =m.codYegua)where  idPoe=".$idpoe." and idUser='".$idProp."' order by 1
				";
		// echo $sql;  
		...
		while($res = mysqli_fetch_array($result)){
			...
			$botonHtml="<label class='btnDel  btn btn-default btn-sm glyphicon glyphicon-trash' data-key=".$res['id']."></label> ";
			if($finalizo==true){
				$botonHtml="";
			}
			 	$checkedMN="  ";
			...
			$html.= "<td align='center'><label  class='btnSearch  btn btn-default btn-sm glyphicon glyphicon-search' data-ctrlset='hidPotro_".$fila."' data-ctrlsetdisplay='txtPotro_".$fila."' ></label></td>";
			...
			$html.= "<td align='center'><label  class='btnSearch btn btn-default btn-sm glyphicon glyphicon-search' data-ctrlset='hidYegua_".$fila."' data-ctrlsetdisplay='txtYegua_".$fila."' ></label></td>";
			$html.= "<td align='left'  >
