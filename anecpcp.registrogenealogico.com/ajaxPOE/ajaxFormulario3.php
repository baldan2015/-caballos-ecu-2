<?php session_start();
	...
	function listar($link,$idpoe,$idProp){
		...
		$html.= "<tr>";
		$html.= "<th style='width:23%;'>Yeguas de mi propiedad y/o cedidas a mi uso</th>";
		$html.= "<th style='width:30px;'>...</th>";
		$html.= "<th style='height:35px;width:23%;'>Potro reproductor</th>";		
		$html.= "<th style='width:30px;'>...</th>";
		...
		$sql = " SELECT m.id,DATE_FORMAT(m.fecServ,'%d/%m/%Y') as fecha,m.metodo,codYegua,codPotro,idPoe,
				...
				ifnull(pad.nombre,(select p.nombre COLLATE utf8_unicode_ci from poe_propiedad p where p.codEjemplar=m.codPotro and  idPoe=".$idpoe." and idUser='".$idProp."')) potro , isTE,DATE_FORMAT(m.fecAborto,'%d/%m/%Y') as fecAborto
