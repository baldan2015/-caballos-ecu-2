<?php session_start();
	...
	function listarNacimiento($link,$idpoe,$idProp){
		...
		$sql = "select m.id,DATE_FORMAT(m.fecha,'%d/%m/%Y') as fecha,m.sexo,m.pelaje,nroReg,m.nombre,codMadre,codPadre,idPoe,
				...
				inner join datos220206 pad on( pad.codigo  =  m.codPadre)
				inner join datos220206 mad on(mad.codigo  =m.codMadre)where idPoe=".$idpoe." and idProp='".$idProp."' order by 1 ";
		// echo $sql ;
		...
		while($res = mysqli_fetch_array($result)){
				...
				$botonHtml="<button class='btnDel' data-key=".$res['id'].">Eliminar	</button>";
			if($finalizo==true){
				$botonHtml="";
			}
		...
		//	$html.= "</td>";
			...
			$html.= "<td align='center'><label  class='btnSearch' data-ctrlset='hidMadre_".$fila."' data-ctrlsetdisplay='txtMadre_".$fila."' >Buscar</label></td>";
			...
			$html.= "<td align='center'><label  class='btnSearch' data-ctrlset='hidPadre_".$fila."' data-ctrlsetdisplay='txtPadre_".$fila."' >Buscar</label></td>";
